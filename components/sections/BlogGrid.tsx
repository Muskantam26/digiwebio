...
export default function BlogGrid({
  ...
  return (
    <section className="py-24 bg-[#0A0B0D] relative" id="blog">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          <div>
            <div className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-[#E2F135] mb-2 font-mono">
              <BookOpen className="w-3.5 h-3.5 text-[#E2F135]" />
              <span>ENGINEERING NOTES & INSIGHTS</span>
            </div>
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-white tracking-tight">
              Modern Web Architecture & Technology Strategy.
            </h2>
          </div>
          {limit && (
            <Link
          ...
          {filtered.map((post, index) => (
            ...
            >
              <div>
                ...
                <div className="bg-[#191B20] p-5 border-b border-[#252830] relative flex items-center justify-between">
                  <span className="px-3 py-1 rounded-full bg-[#0A0B0D] border border-[#252830] text-[#E2F135] text-[10px] font-mono font-bold tracking-wider uppercase">
                    {post.category}
                  </span>
                ...
                <div className="p-6">
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-6">
                    {post.tags.map((tag, idx) => (
                      <span
                        key={idx}
                        className="text-[10px] font-mono bg-[#0A0B0D] text-slate-400 px-2 py-0.5 rounded border border-[#191B20]"
                      >
                        #{tag}
                      </span>
                    ))}
